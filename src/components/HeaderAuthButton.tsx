...
export default function HeaderAuthButton() {
  ...
  if (!configured) {
    return (
      <a
        href="/login"
        className="px-4 py-2 rounded-lg bg-pink-400 text-white font-medium text-sm no-underline hover:bg-pink-500 transition"
      >
        登入
      </a>
    );
    ...
    <div className="relative">
      ...
      >
        {avatarUrl ? (
          ...
          <div className="w-6 h-6 rounded-full bg-pink-100 flex items-center justify-center text-xs font-bold text-pink-500">
            {displayName[0]}
          </div>
        )}
