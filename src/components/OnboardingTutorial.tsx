    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden animate-slideUp">
          ...
          {STEPS.map((_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full border-0 cursor-pointer transition-all ${
                i === step ? "bg-pink-400 w-6" : "bg-slate-200"
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex gap-3 p-4 pt-0">
          ...
          <button
            onClick={handleNext}
            className={`flex-1 py-3 rounded-xl bg-pink-400 text-white font-semibold border-0 cursor-pointer hover:bg-pink-500 transition text-sm ${
              step === 0 ? "w-full" : ""
            }`}
          >
            {isLast ? "開始學習 🚀" : "下一步"}
